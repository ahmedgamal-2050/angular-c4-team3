...
@Component({
  ...
  standalone: true,
  imports: [ProductCardComponent, TranslocoPipe, LucideAngularModule, RouterLink, SlicePipe, StyledHomeTitleComponent],
...
export class MostPopularSectionComponent {
  ...
  selectedOccasion = linkedSignal<string | null>(() => {return this.occasions()[0]?._id ?? null});
  ...
  filteredProducts = computed(() => {
    ...
    return allProducts
      .filter(p => p.occasion === occasionId)
      .slice(0, 12);
  });
