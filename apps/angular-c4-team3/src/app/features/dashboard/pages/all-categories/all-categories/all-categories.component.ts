...
import { occasionsService } from 'apps/angular-c4-team3/src/app/shared/services/occasions.services';
...
import { TableComponent } from 'apps/angular-c4-team3/src/app/shared/components/table/table.component';
import { TableColumn } from 'apps/angular-c4-team3/src/app/shared/components/table/table.modal';
...
export class AllCategoriesComponent implements OnInit, OnDestroy {
  ...
  navigateToAdd(): void {
    this._Router.navigate(['/', APP_ROUTES.DASHBOARD.ROOT, this.entityPath(), 'add']);
  }
  ...
  onDeleteConfirmed(): void {
    ...
    if (!id) return;
    const sub = this.getService().delete(id).subscribe({
      next: () => {
